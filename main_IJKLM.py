...
from IJKLM.run_ampl import run_ampl
...
def run_experiment(
    ...
    for n in N:
        ...
        if below_time_limit(df_ampl, time_limit):
            rr = run_ampl(
                I, J, K, L, M, ijk_tuple, jkl_tuple, klm_tuple, solve, repeats=repeats, number=number)
            df_ampl = process_results(rr, df_ampl)
            print_log_message(language='AMPL', n=n, df=df_ampl)
